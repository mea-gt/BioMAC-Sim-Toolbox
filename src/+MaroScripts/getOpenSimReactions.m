function outData = getOpenSimReactions(filename,osimfile,muscleforce)
...
if exist(name_ext, 'file')
    ...
    result.problem.writeMotionToOsim(result.X,[pathname filesep name]);
    movefile([pathname filesep name '_kinetics_force.sto'], [pathname filesep 'muscleforce.sto']);
    %M% was the purpose of the above line to rename or move the
    %kinetics_force file???

    import org.opensim.modeling.*;

    genericSetupForAn = [curpath filesep 'settings_getReactionForce_generic.xml'];
    ...
    if ~exist([resultspath filesep name '_extfor.xml'],'file')
        ...
        ext_loads.setDataFileName([pathname filesep name '_kinetics_GRFs.mot']);
    ...
    analyzeTool.setCoordinatesFileName([pathname filesep name '_kinematics.mot']);
